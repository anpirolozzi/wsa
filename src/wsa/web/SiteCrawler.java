...
public interface SiteCrawler {
    ...
    static boolean checkDomain(URI dom) {
        try {
            URI uri = (new URI(dom.getScheme(), dom.getUserInfo(), dom.getHost(), dom.getPort(), dom.getPath(), dom.getQuery(), dom.getFragment()));
        } catch (URISyntaxException exc) {return false;}
        return (dom.isAbsolute() && ( dom.getHost()==null || dom.getAuthority().equals(dom.getHost())))&&(dom.getFragment()==null)&&(dom.getQuery()==null);
    }
